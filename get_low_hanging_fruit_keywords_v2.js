...
/**
 * LOW HANGING FRUIT MONEY KEYWORDS - VERSION 2
 * 
 * Modified filters to capture more keywords:
 * - Rank: 11-75 (extended from 11-50) - Pages 2-8
 * - PageSize: 500 (increased from 250) - More results
 * 
 * Compare to V1 to see if broader range yields better opportunities
 */
...
async function getLowHangingFruitKeywordsV2() {
  console.log('═══════════════════════════════════════════════════════════════════════════════');
  console.log('LOW HANGING FRUIT MONEY KEYWORDS V2 - Positions 11-75 (Extended Range)');
  console.log('═══════════════════════════════════════════════════════════════════════════════\n');
  ...
  for (const domain of domains) {
    ...
    try {
      // V2: Rank 11-75 (extended), PageSize 500 (increased)
      const url = `/apis/serp_api/v2/seo/getMostValuableKeywords?query=${domain}&searchVolume.min=10&searchVolume.max=50000&rank.min=11&rank.max=75&costPerClick.min=1&costPerClick.max=1000&costPerClickOption=Exact&pageSize=500&sortBy=ExactCostPerClick`;
  ...
  // Save results
  fs.writeFileSync('low_hanging_fruit_keywords_v2.json', JSON.stringify(results, null, 2));
  console.log('\n\n✅ Full report saved: low_hanging_fruit_keywords_v2.json');

  // Summary
  console.log('\n\n' + '═'.repeat(79));
  console.log('API COST SUMMARY - VERSION 2');
  console.log('═'.repeat(79));
